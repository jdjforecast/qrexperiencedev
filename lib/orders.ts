...
import { OrderSchema, OrdersArraySchema, CreateOrderRpcResultSchema } from "@/types/schemas" // Import Zod schemas

// Define a specific return type for order fetching functions
type OrderFetchResult = { success: true; data: Order } | { success: false; error: string }

// Define a similar type for multiple orders (if needed later, or for consistency)
type OrdersFetchResult = { success: true; data: Order[] } | { success: false; error: string }
